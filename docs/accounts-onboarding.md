...
Migration `a8b9c0d1e2f3` adds three user columns and token/throttle tables. It does not change existing fields or delete data. Account tokens referencing users cascade on user deletion; invitations and throttle hashes have no user ownership and remain for operator retention policy. Expired/consumed tokens never become usable again. No scheduled cleanup or automatic account deletion is introduced. Configure a retention policy and periodic bounded cleanup of expired token/throttle rows before sustained public operation; do not use database resets. Password reset does not delete profile, CV, job or application data.
