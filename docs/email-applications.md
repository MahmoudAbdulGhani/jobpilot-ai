...
The adapter does not retry any send. It classifies documented client-error statuses as rejection and conservatively treats server/network errors as unknown, departing from Google's generic [retry advice](https://developers.google.com/workspace/gmail/api/guides/handle-errors) because blindly retrying a non-idempotent send risks duplicates. Check Gmail manually after uncertainty. Automated reconciliation, inbox access and an unknown-outcome override/resend mechanism are outside this milestone.
