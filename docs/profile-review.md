...
contains `selections` and optional `manual_fields` (a partial candidate profile).
At least one selected suggestion or explicitly supplied manual field is required.
...
manual changes for the same field returns 422. Combined limits and invalid values
