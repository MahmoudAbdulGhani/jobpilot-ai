...
export type Interview={id:string;job_id:string;status:string;revision:number;mode:string;question_count:number;configuration:Config;source_snapshot:Source;turns:Turn[];live_voice?:{status:string;confirmed_turns:{question:string;answer:string}[]|null}|null;guidance:Record<string,string>;actions:Record<string,string>;practice_actions:string[];example_structures:Record<string,string>;practice_priorities:string[];operations:{id:string;step:number;status:string;outcome:string|null;usage:{input_tokens:number;output_tokens:number;reasoning_tokens:number|null}|null}[]};
