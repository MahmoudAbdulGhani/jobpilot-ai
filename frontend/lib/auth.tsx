...
export function AuthProvider({children}:{children:React.ReactNode}){const [user,setUser]=useState<User|null>(null);const [loading,setLoading]=useState(true);const router=useRouter();const path=usePathname();
 ...
 useEffect(()=>{if(!loading&&!user&&!['/','/login','/register','/verify-email','/forgot-password','/reset-password'].includes(path))router.replace('/login');if(!loading&&user&&path==='/')router.replace('/overview');if(!loading&&user&&path==='/login')router.replace(user.onboarding_step && user.onboarding_step!=='done'?'/onboarding':'/overview');},[loading,user,path,router]);
