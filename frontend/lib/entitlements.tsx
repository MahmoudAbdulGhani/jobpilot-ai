...
export type FeatureUsage={label:string;allowance:number;consumed:number;remaining:number;state:'available'|'unavailable'|'not_in_plan'|'exhausted'|'admin';unit:string};
export type Usage={plan:string;base_plan:string;beta_expires_at:string|null;beta_revoked_at:string|null;period_start:string;reset_at:string;reset_timezone:string;total:{allowance:number;consumed:number;remaining:number};profile_refreshes?:{allowance:number|null;consumed:number;remaining:number|null;new_cv_initial_generation:boolean};features:Record<Feature,FeatureUsage>;admin:boolean;billing_available:false;proposed_monthly_price_usd:string;price_note:string;history_note:string};
