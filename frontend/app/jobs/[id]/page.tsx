...
export default function Detail() {
  ...
  const applyUrl = job.source_snapshot?.apply_url && /^https:\/\//i.test(job.source_snapshot.apply_url) ? job.source_snapshot.apply_url : null;
    ...
    <div className="job-workspace">
      <div className="job-reading">
        ...
        <div className="job-tab-panel" id="job-panel-overview" role="tabpanel" aria-labelledby="job-tab-overview" hidden={tab !== 'overview'} tabIndex={0}>
          <section className="description-section workflow-card"><div className="workflow-section-heading"><div><p className="eyebrow">The opportunity</p><h2>About the role</h2></div><FileText size={21} aria-hidden="true" /></div><Description text={job.description} />{sourceWarning&&<p role="alert">{sourceWarning}</p>}{job.source_provider&&job.description&&<p>Review job fit below before creating your tailored CV and cover letter.</p>}{applyUrl&&<p><a className="text-button" href={applyUrl} target="_blank" rel="noopener noreferrer">Apply on employer site <ArrowSquareOut size={14}/></a></p>}</section>
      ...
      <aside className="job-rail" aria-label="Your notes and job details">
        ...
        <section className="saved-details workflow-card"><h2>Saved details</h2><dl><div><dt>Added</dt><dd>{new Date(job.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</dd></div><div><dt>Last updated</dt><dd>{new Date(job.updated_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</dd></div><div><dt>Visibility</dt><dd><LockSimple size={13} />Only you</dd></div></dl>
          {job.source_provider && <p className="source-provenance">Imported from {job.source_provider === 'jobicy' ? 'Jobicy' : job.source_provider==='jobopportunities'?'Job Opportunities API':'JobTech JobSearch'}{job.source_snapshot?.upstream_source?` / ${job.source_snapshot.upstream_source}`:''} / {job.source_external_id}{job.source_snapshot?.test_data ? ' (synthetic test data)' : ''}. Original snapshot retained; edits are yours.</p>}
          {job.source_snapshot && <details className="source-details" open><summary>Imported source details</summary><p>Workplace: {[job.source_snapshot.workplace_city,job.source_snapshot.workplace_region,job.source_snapshot.workplace_country].filter(Boolean).join(', ')||job.location||'Not supplied'}</p><p>Published: {job.source_snapshot.published_at || 'Not supplied'}</p><p>Deadline: {job.source_snapshot.deadline || 'Not supplied'}</p><p>Salary: {job.source_snapshot.salary || 'Not supplied'}</p><p>Workplace model: {job.source_snapshot.workplace_model || 'Not supplied'}{job.source_snapshot.remote_inferred?' (inferred by source)':''}</p><p>Applicant eligibility: {job.source_snapshot.applicant_region || 'Unknown'}. Remote does not mean worldwide eligibility.</p>{snapshotUrl && <a className="text-button" href={snapshotUrl} target="_blank" rel="noopener noreferrer">Original source at import <ArrowSquareOut size={14} /></a>}</details>}
        </section>
