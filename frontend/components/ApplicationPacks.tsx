...
export function ApplicationPacks({ job }: { job: Job }) {
  ...
  async function approve() {
    ...
    try {
      ...
      setApproving(false); setNotice('Version approved. Your separate CV and cover-letter PDFs are ready. DOCX files are also available.'); await load(true);
    } catch (caught) { setError(messageFor(caught)); }
    finally { setBusy(''); }
  }
  ...
  return <section className="application-packs" aria-labelledby="application-packs-heading" aria-busy={!!busy || loading}>
    ...
    {pack && <div className="pack-current">
      ...
      {pack.status === 'failed' && <div className="form-error" role="alert"><p>{pack.outcome_message || 'The provider could not generate valid drafts. Please retry.'}</p><p>No CV or cover letter draft was saved. The private source details below are only the inputs captured before generation.</p></div>}
      ...
      {currentVersion && documents && <>
        ...
        {currentVersion.approved_at && <div className="pack-downloads"><h3>Download approved version {currentVersion.number}</h3><p className="muted pack-help">Each file contains only the approved document. Private evidence and review notes are excluded.</p><div className="pack-actions">{(['cv', 'cover_letter'] as const).map(name => <button className="primary-button" key={`${name}-pdf`} disabled={!!busy} onClick={() => void download(name, 'pdf')}><DownloadSimple size={18} />{name === 'cv' ? 'CV' : 'Cover letter'} PDF</button>)}</div><details className="pack-edit"><summary>Need editable files? Download DOCX</summary><div className="pack-actions">{(['cv', 'cover_letter'] as const).map(name => <button className="secondary-button" key={`${name}-docx`} disabled={!!busy} onClick={() => void download(name, 'docx')}><DownloadSimple size={18} />{name === 'cv' ? 'CV' : 'Cover letter'} DOCX</button>)}</div></details>{dirty && <p className="muted pack-help">These downloads use the approved version, excluding your unsaved edits above.</p>}</div>}
      </>}
      <details className="pack-sources"><summary>Review private source details</summary><p className="muted pack-help">These are the input snapshots captured before generation, not the generated CV or cover letter. Evidence and source text never appear in exports.</p><h3>Confirmed CV</h3><p className="preserve-lines">{pack.source_snapshot.cv_text || 'No source text available.'}</p><h3>Saved profile facts</h3><ul>{pack.source_snapshot.profile_facts?.map(fact => <li key={fact.id}>{fact.value}</li>)}</ul><h3>Skills confirmed for this job</h3><ul>{pack.source_snapshot.application_skills?.map(item => <li key={item.id}>{item.skill}</li>)}</ul><h3>Saved job description</h3><p className="preserve-lines">{pack.source_snapshot.job?.description}</p></details>
