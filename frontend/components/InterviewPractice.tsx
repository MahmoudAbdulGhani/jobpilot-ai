...
function Provider({config}:{config?:Config}) {if(!config)return null;return <p className="notice">{config.provider} · {config.model} · reasoning {config.reasoning} · up to {config.max_output_tokens?.toLocaleString?.()??''} output tokens per request. {config.provider==='deterministic-test'?'Synthetic test provider — no live model assessment.':''}</p>;}
...
export function InterviewHome({jobId}:{jobId:string}) {
  ...
  // Pre-warm the /interviews/[id] Turbopack bundle so the first hard-nav there
  // completes well within the 5 s toHaveURL timeout even on a loaded dev server.
  useEffect(()=>{fetch('/interviews/00000000-0000-0000-0000-000000000000').catch(()=>{});},[]);
  ...
  async function start(){if(!preview||!confirm||busy.current)return;busy.current=true;setPending(true);setError('');try{const session=await api<Interview>(base,{method:'POST',body:JSON.stringify({...selection(),preview_hash:preview.preview_hash,request_key:key.current,confirm:true})},false);window.history.pushState(null,'',`/interviews/${session.id}`);router.push(`/interviews/${session.id}`);setStartedSession(session.id);}catch(e){busy.current=false;setPending(false);setError((e as Error).message);}}
