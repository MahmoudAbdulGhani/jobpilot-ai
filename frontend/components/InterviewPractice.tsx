...
function Provider({config}:{config?:Config}) {if(!config)return null;return <p className="interview-provider">{config.provider} · {config.model} · reasoning {config.reasoning} · up to {config.max_output_tokens?.toLocaleString?.()??''} output tokens per request. {config.provider==='deterministic-test'?'Synthetic test provider — no live model assessment.':''}</p>;}
function Snapshot({source}:{source?:Source}) {if(!source)return null;return <details className="interview-snapshot"><summary>Review captured source information</summary><h3>{source.job?.title} · {source.job?.company}</h3><h4>Job description</h4><p className="preserve-lines">{source.job?.description}</p><h4>{source.source_kind==='approved_pack'?'Approved CV':'Reviewed CV'}</h4><p className="preserve-lines">{source.cv_text}</p>{source.cover_letter_text&&<><h4>Approved cover letter</h4><p className="preserve-lines">{source.cover_letter_text}</p></>}<h4>Candidate profile snapshot</h4><dl>{Object.entries(source.candidate_profile||{}).filter(([,v])=>v!==null).map(([k,v])=><div key={k}><dt>{label(k)}</dt><dd className="preserve-lines">{readable(v)}</dd></div>)}</dl></details>;}
...
export function InterviewHome({jobId}:{jobId:string}) {
  ...
  async function start(){if(!preview||!confirm||busy.current)return;busy.current=true;setPending(true);setError('');try{const session=await api<Interview>(base,{method:'POST',body:JSON.stringify({...selection(),preview_hash:preview.preview_hash,request_key:key.current,confirm:true})},false);router.push(`/interviews/${session.id}`);setStartedSession(session.id);}catch(e){busy.current=false;setPending(false);setError((e as Error).message);}}
  if(startedSession)return <InterviewSessionView id={startedSession}/>;
  return <section className="interview-page app-page"><Link className="back-link" href={`/jobs/${jobId}`}>Back to saved job</Link><PageHeader eyebrow="Prepare your next conversation" title="Interview practice" subtitle="Practice with your reviewed material. Choose a source, review what will be shared, then start when you’re ready."/><p>Private interview practice, not a hiring prediction. Optional voice input is available inside a session; the full text workflow remains available.</p>
    ...
    {options&&<><Provider config={options.configuration}/>{!options.available&&<p role="alert">{options.message}</p>}
      <form className="interview-settings" onSubmit={e=>{e.preventDefault();void review();}}><fieldset disabled={pending}><legend>Practice settings</legend>
...
export function InterviewSessionView({id}:{id:string}) {
  ...
  useEffect(()=>{void load();},[load]);
  async function save(current:Interview){const result=await api<Interview>(`/interviews/${id}/answer`,{method:'PATCH',body:JSON.stringify({revision:current.revision,question_number:current.turns?.length||0,answer})},false);display(result);return result;}
  ...
  if(!session)return <section className="interview-page app-page">{error?<p role="alert">{error}<button onClick={()=>void load()}>Reload session</button></p>:<p role="status">Loading interview…</p>}</section>;
  const current=session.turns?.at(-1),completed=session.status==='completed',waiting=pending||session.status==='generating';
  return <section className="interview-page app-page" aria-busy={pending}><Link className="back-link" href={`/jobs/${session.job_id}/interviews`}>Back to practice sessions</Link><PageHeader eyebrow="Your interview workspace" title={completed?'Interview review':'Interview practice session'}/><Provider config={session.configuration}/><p>AI-generated practice guidance, not a hiring probability or verified assessment of your abilities.</p><Snapshot source={session.source_snapshot}/>
    <p>Status: {session.status} · {session.turns?.length||0} of {session.question_count} questions</p>
    ...
    {(session.turns||[]).filter(t=>completed||t.feedback).map(t=><article className="interview-feedback-card" key={t.number}><h3>Question {t.number}</h3><p>{t.text}</p><blockquote>{t.question.quote}</blockquote><h4>Your saved answer</h4><p className="preserve-lines">{t.answer}</p><Feedback turn={t} session={session}/></article>)}
