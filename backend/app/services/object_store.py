...
class StorageUnavailable(Exception):
    def __init__(self, message="Private document storage is unavailable", *, category=None):
...
class SupabaseStore:
    ...
    def request(self, method, path, **kwargs):
        ...
        try:
            ...
            raise StorageUnavailable(category=cat) from exc
    ...
    def write(self, storage_id, data):
        if len(data) > self.settings.RESUME_MAX_SIZE_MB * 1024 * 1024:
            ...
            raise StorageUnavailable(category="file_too_large")
        self.private()
